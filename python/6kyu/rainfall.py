...
def mean(town, strng):
  ...
  # if the sunstring is the very end of the data, no "\n"
  if (sub_string == None):
    sub_string = re.search("{}:(.+?)$".format(town), strng)

  if (sub_string == None):
    return -1

  month_data_list = sub_string.group(1).split(',')
  rain_data_list = [float(x[4:]) for x in month_data_list]
  mean = sum(rain_data_list)/len(rain_data_list)
  return mean
...
def variance(town, strng):
  ...
  for x in rain_data_list:
    variance += ((x-mean)**2)
  variance = variance/len(rain_data_list)
